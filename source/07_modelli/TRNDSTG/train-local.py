...
#
# sys.path.append('C:\\Users\\mantioco\\Documents\\Progetto EVA\\corsacontroiltempo')
...
for COD_SIMULAZIONE, DATA_CONSUNTIVO, DELTA_HOURS in RUN_PARAMETERS:
    ...
    for cluster_path in CLUSTERS:
        ...
        for period in range(2, min(24 * 32, int(np.floor(df_train.shape[0] / 2))), 2):
            seasonal_dec_result = seasonal_decompose(df_train['CONSUMI'], model='addittive', two_sided=False, period=period)

            trend = seasonal_dec_result.trend
            seasonal = seasonal_dec_result.seasonal

            max_trend_day = max(trend.index).to_pydatetime()
            for pred_iter in range(DELTA_HOURS):
            ...
            prediction_trend = trend.loc[trend.index > max_trend_day]
            ...
            best_result.append(
                [period, 'addittive', mape(prediction['CONSUMI'].values, prediction['PREVISIONE'].values),
                 np.sum(np.abs(prediction['CONSUMI'].values - prediction['PREVISIONE'].values))/np.sum(prediction['CONSUMI'].values)]
            )

        best_result = pd.DataFrame(best_result, columns=['PERIODO', 'TIPO_MODELLO', 'MAPE', 'WMAE'])
        best_result = best_result.loc[[best_result['WMAE'].idxmin()]]
